...
func (v *Virter) VMList() ([]Domain, error) {
	domains, _, err := v.libvirt.ConnectListAllDomains(-1, 0)
	if err != nil {
		return nil, fmt.Errorf("failed to list available networks: %w", err)
	}

	xmllist := make([]Domain, len(domains))
	//statelist := make([]int32, len(domains))
	for i, domain := range domains {
		desc, err := v.libvirt.DomainGetXMLDesc(domain, 0)
		if err != nil {
			return nil, fmt.Errorf("failed to fetch domain xml '%s': %w", domain.Name, err)
		}

		err = xml.Unmarshal([]byte(desc), &xmllist[i])
		if err != nil {
			return nil, fmt.Errorf("failed to convert domain '%s' xml to go object: %w", domain.Name, err)
		}

		state, err := v.libvirt.DomainIsActive(domain)
		if err != nil {
			return nil, fmt.Errorf("failed to fetch domain state '%s': %w", domain.Name, err)
		}
		xmllist[i].Status = state
	}
